...
function build_packages() {
	...
	sed -i "s|self._rt.set_user('shrinkwrap')|self._rt.environment = { 'FORCE_UNSAFE_CONFIGURE': '1' }|g" shrinkwrap/shrinkwrap/utils/runtime.py
...
function prepare_rootfs() {
	...
	# mount rootfs
	trap unmount ERR
	rootfs=$(pwd)/package/cca-3world/rootfs.ext2
	mkdir -p $ROOT
